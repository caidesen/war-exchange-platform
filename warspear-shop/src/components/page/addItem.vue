<template>
  <div id="addItem">
    ...
    <mt-field label="添加描述" placeholder="添加描述........" type="textarea" rows="6" v-model="description"></mt-field>
    ...
    <div>
      <p>注意事项：</p>
      <p>
        <span>1. 发布和上传图片需要注册并完成激活后才能使用。</span><br>
        <span>2. 标题尽量简短，过长可能会影响页面显示效果。</span><br>
        <span>3. 如果是出金收金，直接在标题中直观写出比例。</span>
      </p>
    </div>
...
export default {
  ...
  methods: {
    ...
    pushItem () {
      ...
      if (this.$store.state.addItem.exchangeType === '金币') {
        pics = []
      }
      let item = {
        title: this.title,
